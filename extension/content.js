...
export let CONTENT = function(config, pack) {
	...
	game.qhly_getViewSkills = function(name) {
		...
		for (var skill of skills) {
			...
			if (info.derivation) {
				if (typeof info.derivation === 'string' && lib.skill[info.derivation]) viewSkills.add(info
					.derivation);
				else {
	...
	*/
	...
	game.qhly_open_new = function(name, page, ingame) {
		...
		var setSize = function() {
			...
			if (lib.config.qhly_currentViewSkin != 'decade' && lib.config.qhly_currentViewSkin !=
				'shousha') {
			...
			if (lib.config.qhly_currentViewSkin == 'shousha') {
				backButton.style.transform = 'translateY(' + (Math.round(height) - document.body
					.offsetHeight) * 0.5 + 'px)';
				dibuhuo.style.transform = 'translateY(' + (document.body.offsetHeight - Math.round(
					height)) * 0.5 + 'px)';
			}
