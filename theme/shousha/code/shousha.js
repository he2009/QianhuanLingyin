...
window.qhly_import(function(lib, game, ui, get, ai, _status){
    game.qhly_initShoushaView = function (name, view, page, cplayer) {
        ...
        var Bgbody = ui.create.div('.qh-shoushabgbody', view);
        ...
        /*ui.create.div('.qh-shoushabgtail', view);*//*已弃用*/
        var refreshRank = function () { };
        subView.avatar = ui.create.div('.qh-shousha-big-avatar', view);/*此处为角色框体大图*/
        if (lib.config.qhly_lutouType && lib.config.qhly_lutouType == 'shousha') 
          subView.avatar.classList.add('shousha');
        ...
        if (game.qhly_getPlayerStatus(cplayer, cplayer && cplayer.name2 && cplayer.name2 == name) == 2) 
          game.qhly_setPlayerStatus(subView.avatarImage, undefined, 2);
        ...
        subView.hpborder = ui.create.div('.qh-hp-border',  view);
        subView.bigwrapborder = ui.create.div('.qh-decade-big-wrap-border',subView.avatar);
       ...
       if (dibuhuo) {
        ...
        subView.dynamicToggle.addEventListener(lib.config.touchscreen ? 'touchend' : 'click', function () {
          ...
          if (this.classList.contains('jing')) {
            ...
            if (lib.config.qhly_skinset.djtoggle[name] && lib.config.qhly_skinset.djtoggle[name][skinStr])
               delete lib.config.qhly_skinset.djtoggle[name][skinStr];
            if (state.mainView.avatarImage.dynamic && state.mainView.avatarImage.dynamic.primary && state.mainView.avatarImage.dynamic.primary.name)
               _status.currentTexiao = state.mainView.avatarImage.dynamic.primary.name;
            if (_status.currentTexiao) {
              state.mainView.avatarImage.addEventListener('click',() => {
            ...
            if (state.mainView.avatarImage.stopDynamic) 
              state.mainView.avatarImage.stopDynamic();
            if (!lib.config.qhly_skinset.djtoggle[name]) 
              lib.config.qhly_skinset.djtoggle[name] = {};
            lib.config.qhly_skinset.djtoggle[name][skinStr] = true;
          }
          game.qhlySyncConfig();
          if (!_status['qhly_primarySkin_' + name] || _status['qhly_primarySkin_' + name] && _status['qhly_primarySkin_' + name] == game.qhly_getSkin(name)) 
        ...
        var defaultButton2 = ui.create.div('.qh-avatar-default2', shoushaButtons);
        ...
        defaultButton.listen(function () {
          ...
          if (currentSkin) {
            game.qhly_setCurrentSkin(name, currentSkin.skinId, function () {
              ...
              game.qhly_changeDynamicSkin(name);
              game.playAudio('../extension/千幻聆音/audio/qhly_voc_click3.mp3');
        ...
        subView.page = {
          ...
          skill: {
            ...
            init: function (name, state) {
              ...
              let Allbuttons = document.querySelectorAll(".qh-button");
             ...
             /* this.voice.listen(function () {
                ...
                taiciblackBg.listen(function () {
                  that.taici.hide();
                  if (_status.qhly_shoushaBigVoice) 
                  delete _status.qhly_shoushaBigVoice;
                  this.remove();
                })
                that.taici.show();
              })*/
              ...
              for (var skill of viewSkill) {
                ...
                if (detail) {
                  ...
                  if (get.info(skill) && get.info(skill).trigger) 
                    zhubeidong.setBackgroundImage("extension/千幻聆音/theme/shousha/beidong.png");
                  else 
                  zhubeidong.setBackgroundImage("extension/千幻聆音/theme/shousha/zhudong.png");
              ...
              var bindFunc = function (checkbox, text) {
                ...
                game.playAudio('../extension/千幻聆音/audio/qhly_voc_check.mp3');
          ...
          skin: {
            ...
            onClickSkin: function (num, name, state) {
              ...
              if (_status['qhly_primarySkin_' + name] !== undefined) {
                if (skin.skinId == _status['qhly_primarySkin_' + name]){
                  defaultButton.hide();
                  defaultButton2.show();
                 }
                else{
            ...
            refreshAfterGot: function (name, state) {
              ...
              if (this.firstRefresh) {
                ...
                for (var i = 0; i < this.skinList.length; i++) {
                  ...
                  skinView.belowText.setAttribute('data-camp', game.qhly_getSkinLevel(name, skin));
                  ...
                  if (skin) {
                    ...
                    if (info) {
                      ...
                      if (game.qhly_skinIs(name, skin)) {
                        ...
                        state.mainView.avatar.belowText.innerHTML = (info.translation + ' ' );
                    ...
                    if ((!lib.config.qhly_skinset.djtoggle[name] || lib.config.qhly_skinset.djtoggle[name] && !lib.config.qhly_skinset.djtoggle[name][skin.substring(0, skin.lastIndexOf('.'))]) && window.decadeUI && decadeUI.dynamicSkin && decadeUI.dynamicSkin[name] && Object.keys(decadeUI.dynamicSkin[name]).contains(info.translation)) {
                      if (game.qhly_skinIs(name, skin)) {
                        ...
                        if (state.mainView.avatarImage.dynamic && state.mainView.avatarImage.dynamic.primary && state.mainView.avatarImage.dynamic.primary.name) 
                          _status.currentTexiao = state.mainView.avatarImage.dynamic.primary.name;
                        if (_status.currentTexiao) {
                          state.mainView.avatarImage.addEventListener('click',(event) => {
                            clearTimeout(clickTimeId);
                            clickTimeId = setTimeout(()=>{
                              game.playShoushaAvatar(state.mainView.avatarImage, lib.config.qhly_shoushaBigFlip[name][game.qhly_getSkin(name)], state.name);
                            },220)
                    ...
                    if (game.qhly_skinIs(name, skin)) {
                      state.mainView.avatar.belowText.innerHTML = ("经典形象" + ' ');
                      state.mainView.dragontail.show();
                      state.mainView.skinType.show();
                      state.mainView.hp.show();
                      state.mainView.hpWrap.show();
                      ...
                      if (this.skinList[0].skinId == null && this.skinList[0].bothSkin && lib.config.qhly_skinset && lib.config.qhly_skinset.djtoggle && (!lib.config.qhly_skinset.djtoggle[name] || lib.config.qhly_skinset.djtoggle[name] && !lib.config.qhly_skinset.djtoggle[name]['经典形象']))
                         game.qhly_changeDynamicSkin(state.mainView.avatarImage, '经典形象', name);
                    }
                  ...
                  skinView.listen(function () {
                    ...
                    if (!this.isLocked) {
                      ...
                      clearInterval(_status.texiaoTimer);
                      clearTimeout(_status.texiaoTimer2);
                      state.mainView.avatar.belowText.innerHTML = (this.belowText.innerHTML + ' ');
                      state.mainView.hpWrap.style.width = state.mainView.avatar.belowText.innerHTML.length * 1.25 + 'em';
                      var extInfo = "";
                      ...
                      game.qhly_setCurrentSkin(name, originSkin, function () {
                        if (now.belowText.innerHTML != '经典形象') {
                          ...
                          else if (state.mainView.avatarImage.stopDynamic) 
                            state.mainView.avatarImage.stopDynamic();
                          if (that.skinList[now.id.slice(12)].bothSkin && window.decadeUI && decadeUI.dynamicSkin && decadeUI.dynamicSkin[name] && Object.keys(decadeUI.dynamicSkin[name]).includes(now.belowText.innerHTML)) {
                            state.mainView.dynamicToggle.setAttribute('toggle', true);
                            if (lib.config.qhly_skinset && lib.config.qhly_skinset.djtoggle && (!lib.config.qhly_skinset.djtoggle[name] || lib.config.qhly_skinset.djtoggle[name] && !lib.config.qhly_skinset.djtoggle[name][now.belowText.innerHTML]))
                              state.mainView.dynamicToggle.classList.remove('jing');
                            else state.mainView.dynamicToggle.classList.add('jing');
                          }
                          else state.mainView.dynamicToggle.setAttribute('toggle', false);
                        } else {
                         /* state.mainView.dragontail.show();
                          ...
                          state.mainView.hp.show();
                          state.mainView.hpWrap.show();*/
                          if (that.skinList[0].skinId == null && that.skinList[0].bothSkin && lib.config.qhly_skinset && lib.config.qhly_skinset.djtoggle && (!lib.config.qhly_skinset.djtoggle[name] || lib.config.qhly_skinset.djtoggle[name] && !lib.config.qhly_skinset.djtoggle[name]['经典形象']))
                             game.qhly_changeDynamicSkin(state.mainView.avatarImage, '经典形象', name);
                          else if (state.mainView.avatarImage.stopDynamic) 
                            state.mainView.avatarImage.stopDynamic();
                          if (that.skinList[0].skinId == null && that.skinList[0].bothSkin) {
                            state.mainView.dynamicToggle.setAttribute('toggle', true);
                            if (lib.config.qhly_skinset && lib.config.qhly_skinset.djtoggle && (!lib.config.qhly_skinset.djtoggle[name] || lib.config.qhly_skinset.djtoggle[name] && !lib.config.qhly_skinset.djtoggle[name]['经典形象'])) 
                              state.mainView.dynamicToggle.classList.remove('jing');
                            else state.mainView.dynamicToggle.classList.add('jing');
                          }
                          else state.mainView.dynamicToggle.setAttribute('toggle', false);
            ...
            init: function (name, state) {
              ...
              /*ui.create.div('.qh-skinchange-decade-big-dibuhuo', dialog);*//*手杀特色，已弃用*/
              ...
              /*新代码*/
              conversion.addEventListener('click',conversion1)
              let isMovingRight = true; 
              function conversion1(){
                if(isMovingRight){
                  dialog.setAttribute('style','left:-88%');
                  shoushaBg.setAttribute('style','display:none;');
                  subView.avatar.setAttribute('style','left:59%;');
                  subView.hp.setAttribute('style','display:none;');
                  subView.hpborder.setAttribute('style','opacity:0;');
                  state.mainView.avatarImage.removeEventListener('dblclick',variation);
                  subView.skinType.setAttribute('style','display:none');
                  /*BackGround.setAttribute('style','z-index:80');*/
                  isMovingRight = false; 
                }else if(!isMovingRight){
                  dialog.setAttribute('style','left:0');
                  shoushaBg.setAttribute('style','display:block;');
                  subView.avatar.setAttribute('style','rigth:59%;');
                  subView.hp.setAttribute('style','display:block;');
                  subView.hpborder.setAttribute('style','opacity:1;');
                  state.mainView.avatarImage.addEventListener('dblclick',variation);
                  subView.skinType.setAttribute('style','display:block');
        ...
        refreshRank = function () {
          ...
          if (subView.skinRank) {
            if (game.qhly_getSkin(state.name)) {
              subView.rank.show();
              subView.skinRank.show();
              var rarity2 = game.getRarity(state.name);
              if (rarity2) {
                subView.rank.setBackgroundImage('extension/千幻聆音/image/rarity_' + game.getRarity(state.name) + '.png');
                }
            ...
            subView.pageButton[m].listen(function () {
              if (subView.currentPage != m) {
                ...
                if (state.extraMenu) {
                 ...
                 game.playAudio('../extension/千幻聆音/audio/qhly_voc_dec_press.mp3');
              } else if (m == 'introduce') {
                if (state.extraMenu) {
                  state.extraMenu.delete();
                  document.getElementsByClassName('qh-otherinfoarrow')[0].classList.remove('sel');
                  ...
                  if (extra) {
                  game.playAudio('../extension/千幻聆音/audio/qhly_voc_click2.mp3');
                    var arr = [{
                      name: '简介',
                      onchange: function () {
                        document.getElementsByClassName('qh-otherinfoarrow')[0].classList.remove('sel');
          ...
          const prefixList = ['SP', 'OL', 'TW'];
            ...
            if (k > -1) {
              for (var j = 0; j < cleanName.length; j++) {
                  ...
                  ui.create.div('.qh-shoushaName-other', subView.name, cleanName.charAt(j));
                }
                }
              }
          }
...
        /*以下为新加代码*/
        ...
        var conversion = ui.create.div('.qh-avatar-taici', shoushaButtons);
        var abo = true;
        state.mainView.avatarImage.addEventListener('dblclick',variation);
          function variation(){
            clearTimeout(clickTimeId);
            if(abo){
              if(!document.getElementById('qhlySkinChangebutton1')){
                let url = subView.avatarImage.style.getPropertyValue('background-image').match(/\"([^\"]*)\"/)[1];
                getImageSize(url, function(size) {
                let width1 = size.width;
                let height1 = size.height;
                const radios = width1/height1;
                   if(radios <= 1){
                    if( state.mainView.dynamicToggle.classList.contains('jing')||subView.avatarImage.dynamic && subView.avatarImage.dynamic.primary == null){
                      state.mainView.avatarImage.setAttribute('style', 'width:112%;height: calc(101%);'); 
                      subView.skinType.setAttribute('style','display:none');
                      subView.rank.setAttribute('style','display:none');
                      subView.skinRank.setAttribute('style','display:none');
                      subView.hpWrap.setAttribute('style','display:none');
                      subView.hp.hide();
                      abo = false;
                    }
                  }
                ...
                state.mainView.avatarImage.setAttribute('style', 'width:calc(252%);'); 
                subView.hp.hide();
                subView.skinType.setAttribute('style','display:none');
                subView.rank.setAttribute('style', 'left: 118%;');
                subView.skinRank.setAttribute('style', 'right:-151% ');
                subView.hpWrap.setAttribute('style','left:120%');
                subView.bigwrapborder.setAttribute('style','display:none');
                conversion.style.display = 'none';
                abo = false;
                refreshRank();
                refreshView(state, subView);
            }else{
                state.mainView.avatarImage.setAttribute('style', 'width:calc(99%);');
                subView.hp.show();
                subView.skinType.setAttribute('style','display:block');
                subView.rank.setAttribute('style', 'left: 38%;');
                subView.skinRank.setAttribute('style', 'right:1% ');
                subView.hpWrap.setAttribute('style','left: 38%;');
                subView.bigwrapborder.setAttribute('style','display:block');
         ...
         /* subView.avatar.addEventListener(lib.config.touchscreen ? "touchstart" : "mousedown", (event) => {
          const t = event.touches[0]; 
          startX = t.clientX;
          huadong = true; 
        });
        subView.avatar.addEventListener(lib.config.touchscreen ? "touchmove" : 'mousemove', (event) => {
          if (!huadong) return; 
          const t = event.touches[0];
